...
from utils.image_utils import (apply_clahe, apply_stretch,
                                        convert_to_8bit, sharpen_image)
...
class Georeferencing:
    ...
    def __init__(
        ...
        self.output_path = os.path.join(output_path, "04_Georeferencing")
        os.makedirs(self.output_path, exist_ok=True)
        self.output_epsg = output_epsg
        self.binning = binning
        
        self.raw_image_list = sorted(glob.glob(os.path.join(self.input_path, "*.tiff")))
    ...
    def read_imu_data(self, imu_file: str) -> pd.DataFrame:
        ...
        try:
            ...
            LOG.error(f"Error reading IMU data: {e}")
            return pd.DataFrame()

    def calculate_rotation_matrix(self, roll: float, pitch: float, yaw: float) -> np.ndarray:
        """
    ...
    def calculate_effective_gsd(self, focal_length: float, altitude: float, pixel_size: float) -> float:
        """
            ...
            try:
                with rio.open(img_path) as src:
                    ...
                    if imu_data is not None:
                        ...
                        timestamp = float(os.path.basename(img_path).split("_")[0])
                        closest_idx = (imu_data["timestamp"] - timestamp).abs().idxmin()
                        row = imu_data.iloc[closest_idx]
                        
                        # Calculate rotation matrix
                        R = self.calculate_rotation_matrix(row["roll"], row["pitch"], row["yaw"])
                        
                        # Update transform with rotation
                        # This is a simplified example - actual implementation would depend on
                        # specific requirements and coordinate system transformations
                        transform = transform * rio.transform.Affine.rotation(
                            np.deg2rad(row["yaw"])
                        )
                    ...
                    output_file = os.path.join(
                        output_path,
                        f"georef_{os.path.basename(img_path)}"
                    )
                    ...
                    vis_file = os.path.join(
                        output_path,
                        f"georef_{os.path.splitext(os.path.basename(img_path))[0]}_8b.png"
                    )
                    cv2.imwrite(
                        vis_file,
                        convert_to_8bit(apply_stretch(sharpen_image(image)))
                    )
    ...
    def run_georeferencing(self, imu_file: Optional[str] = None) -> None:
        """
