...
LOG = logging.getLogger("Inter-Frame Registration Logger")  # Logger
...
class InterFrameRegistration:
    ...
    def __init__(
        ...
        self.output_path = os.path.join(output_path, "03_Inter_Frame_Registration")
        os.makedirs(self.output_path, exist_ok=True)
        self.binning = binning
        
        self.out_folder_mosaic = os.path.join(self.output_path, "01_Mosaic")
    ...
    def feature_matching(self, img_ref, img_tar, dist, match_cond=False):
        """
        Performs feature matching between reference and target images.

        Args:
            img_ref: Reference image
            img_tar: Target image
            dist: Distance threshold for filtering matches
            match_cond: Whether to generate mosaic after matching

        Returns:
            Tuple containing:
                - Stitched mosaic (if match_cond is True)
                - Shift coordinates (dx, dy)
                - Homography matrix
        """
    ...
    def inter_frame_registration(self, channel):
        ...
        if not raw_image_list:
            LOG.warning(f"No images found for channel {channel}")
            return
